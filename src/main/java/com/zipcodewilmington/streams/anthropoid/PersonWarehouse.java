...
public final class PersonWarehouse implements Iterable<Person> {
    ...
    Set<String> unique=new HashSet();
        return  people.stream().filter(person -> unique.add(person.getName()));
    }
    ...
    public Stream<Person> getFirstNUniquelyNamedPeople(int n) {
        Set<String> unique=new HashSet();
        return  people.stream().filter(person -> unique.add(person.getName())).limit(n);
    }
    ...
    public Stream<Stream<String>> getNestedAliases() {
        ...
        return people.stream().map(Person::getAliases).map(Arrays::stream);
    }
